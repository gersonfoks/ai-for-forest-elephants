...
It in turn is also based and referred from https://github.com/microsoft/Swin-Transformer
...
class WindowAttention(pl.LightningModule):
...
class PatchMerging(pl.LightningModule):
...
class BasicLayer(pl.LightningModule):
...
# The Core of HTSAT
class HTSAT_Swin_Transformer(pl.LightningModule):
    ...
    def __init__(self, spec_size=256, patch_size=4, patch_stride=(4, 4),
        ...
        if self.config.swin_pretrain_path is not None:
            ckpt = torch.load(self.config.swin_pretrain_path, map_location="cpu")
            # load pretrain model
            ckpt = ckpt["model"]
            found_parameters = []
            unfound_parameters = []
            model_params = dict(self.state_dict())
    ...
    def forward_features(self, x):
        ...
        fpx = fpx.permute(0, 2, 1)  # Swap B,T,C to B,C,T back.
    ...
    def infer(self, x: torch.Tensor):
        """
            The model by default outputs 1024 timesteps - to use it in the same way as AST is used,
            with timesteps being 1 second long, we aggregate the output of the model here,
            taking the mean probabilities for each timestep.

            To easily convert 1024 timesteps to 10, 2 frames are discarded from the beginning and end,
            to make it divisible by 10.
        """

        def aggregate_output(output):
            discard_frames = 2
            # Discard the extra frames from each end of the input
            output = output[:, :, discard_frames:-discard_frames]
            # Reshape the output to (batch, 2, 10, 102) by splitting the last dimension into 10 non-overlapping segments
            output_reshaped = output.view(output.shape[0], output.shape[1], 10, -1)
            # Take the mean of each segment along the last dimension to get the output of shape (batch, 2, 10)
            output_aggregated = torch.mean(output_reshaped, axis=-1)
            return output_aggregated
        with torch.no_grad():
            pred = self.forward(x)['framewise_output']
        return aggregate_output(pred)

    def training_step(self, batch, batch_idx):
        pred = self(batch[0])['framewise_output']
        loss = self.loss_func(pred, batch[1])

        self.log("train_loss", loss, on_step=True,
                 on_epoch=True, prog_bar=True, logger=True)

        if self.train_metrics:
            self.log_metrics(batch[1], pred, self.train_metrics)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        if self.val_metrics:
            self.log_metrics(batch[1], pred, self.val_metrics)

        return [pred.detach(), batch[1].detach()]
